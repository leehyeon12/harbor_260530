...
- 실습: 나이 계산기(`1_age-calculator`), D-day 카운터(`2_dday-counter`), 색상 팔레트(`3_color-palette`)
...
### 외부 API 활용 앱
- `apod-magazine` (NASA 오늘의 천문 사진), `weather-today`(날씨), `pokebook`(포켓몬 도감)
...
## 자주 나올 질문 → 어디를 보면 되는지
- "정적 파일 서버 어떻게 만들었어?" → `week_3/webserver-01/server.js`
- "API 엔드포인트(라우팅) 예시 보여줘" → `week_3/webserver-02/server.js`
- "비밀번호/인증 처리 어떻게 했고 보안 주의점은?" → `week_3/webserver-03/server.js`
