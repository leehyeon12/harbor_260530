...
// ========================================
// 🖼️ ImageKit 업로드 (상품 이미지)
// ========================================
// 브라우저는 우리 서버로만 base64 이미지를 보내고, PRIVATE 키는 서버에만 존재한다.
// (키가 브라우저에 노출되면 누구나 우리 ImageKit 계정을 조작할 수 있으므로 서버 경유 업로드)
...
async function uploadToImageKit(base64Data, fileName) {
  ...
  // ImageKit의 file 필드는 data URI 접두사("data:image/png;base64,") 없는 순수 base64를 기대한다
  ...
  // 업로드된 이미지의 공개 URL 만 반환 (이 URL 을 DB 에 저장)
  return json.url
}

// POST /api/products { name, price, description, fileName, fileBase64 }
// 이미지를 ImageKit 에 올린 뒤, 받은 URL 로 상품을 DB 에 등록한다.
// 별도 관리자 role 없이 "로그인한 사용자면 등록 가능"한 학습용 간단 버전.
async function handleCreateProduct(req, res) {
  ...
  // 입력 검증 (프론트에서도 막지만, 서버도 반드시 다시 검증한다)
  if (name === '') {
    sendJson(res, 400, { error: '상품명을 입력하세요' })
    return
  }
  if (!Number.isInteger(price) || price <= 0) {
    sendJson(res, 400, { error: '가격은 양의 정수여야 합니다' })
    return
  }
  if (fileBase64 === '') {
    sendJson(res, 400, { error: '상품 이미지를 첨부하세요' })
    return
  }
  // 서버리스 본문 크기 한계를 고려해 base64 기준 약 7MB(원본 약 5MB)로 제한
  if (fileBase64.length > 7 * 1024 * 1024) {
    sendJson(res, 413, { error: '이미지 용량이 너무 큽니다 (5MB 이하로 올려주세요)' })
    return
  }

  // 1) 이미지를 ImageKit 에 업로드 → 공개 URL 획득
  let imageUrl
  try {
    imageUrl = await uploadToImageKit(fileBase64, body.fileName || name)
  } catch (err) {
  ...
  // 2) 상품을 DB 에 저장 (image_url 은 방금 받은 URL)
  const result = await pool.query(
    'INSERT INTO harbor_w5_shop_products (name, price, image_url, description) VALUES ($1, $2, $3, $4) RETURNING id, name, price, image_url, description',
