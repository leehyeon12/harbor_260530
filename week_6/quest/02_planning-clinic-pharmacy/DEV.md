...
### Supabase Auth
- 방식: **이메일 매직링크(OTP)** 우선(비밀번호 관리 부담 없음, 초보 타겟에 적합). 여력 되면 카카오 OAuth 추가(선택).
- 프론트: `supabase.auth.signInWithOtp({ email })` / `onAuthStateChange`로 세션 감지 → `AuthBar` 컴포넌트에서 로그인 상태·로그아웃 표시.
...
- [ ] 🟡 `AuthBar` 컴포넌트 — 이메일 매직링크 로그인/로그아웃, `onAuthStateChange` 세션 감지 (기능 5)
...
| Supabase Auth Redirect URL | 매직링크용 Site URL·Redirect URL에 로컬(`http://localhost:3000`)·배포 URL 등록 |
...
| 항목 | 설명 |
|------|------|
| 카카오 OAuth | 매직링크 외 카카오 로그인 추가 시(초보 타겟 전환율↑). Supabase Auth Provider 등록 필요 |
